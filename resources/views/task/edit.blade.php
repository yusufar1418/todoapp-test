                    ...
                    <form method="POST" action="{{ route('task.update', $task) }}" enctype="multipart/form-data">
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="description" class="block mt-1 w-full" type="text" name="description"
                                value="{{ old('description', $task->description) }}" autocomplete="description" />
                            ...
                            <select name="user_ids[]" id="user_ids"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ in_array($user->id, old('user_ids', $task->users->pluck('id')->toArray())) ? 'selected' : '' }}
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="task_at" class="block mt-1 w-full" type="datetime-local" name="task_at"
                                value="{{ old('task_at', $task->task_at->format('Y-m-d\TH:i')) }}" required />
